...
AOARange = np.arange(-1.5, 1.5, 0.1)


chord_length_m = 0.125
flow_speed_m_s = 20/3.6 # 20km/h --> m/s
...
directory = 'profiles_to_consider'
profiles = [f for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f))]

#profiles = ['n0011sc.dat']
for i, foilname in enumerate(profiles):
    foilpath = directory + '/' + foilname
    foilname = foilpath.split('.')[0]
    print(f"Foil {i}/{len(profiles)} with path: {foilpath}")

    foil = vft.repanel(vft.read_selig(foilpath), 175)
    results[foilname] = {}
